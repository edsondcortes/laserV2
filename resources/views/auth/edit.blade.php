            ...
            <div class="card-content">
                ...
                <div class="row">
                    <div class="col s12" id="account">
                        <form id="userCreate" method="post" action="{{ route('usuarios.update', $user->id) }}">
                            @csrf
                            @method('put')
                            <div class="row">
                                <div class="col s12">
                                    <div class="row">
                                        ...
                                        <div class="col s12 input-field">
                                            <input id="password_confirmation" name="password_confirmation" type="password" class="validate"
                                                   data-error=".errorPasswordConfirmation">
                                        ...
                                        <div class="col s12">
                                            ...
                                            <select id="role" name="role" class="select2 browser-default" required>
                                                @foreach($roles as $role)
                                                    <option value="{{ $role->name }}" {{ $user->hasRole($role->name) ==  $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
