...
<div class="card">
    <div class="card-content">
        <div class="table-responsive">
            @if ($budgets == null)
                <h5>Não existem orçamentos a serem editados no momento</h5>
            @else
                <div class="row">
                    <div class="col s12 rigth-align">
                        <form class="search-element" method="GET" action="{{ route('edition.index')}}">
                            <div class="input-field mb-0 mt-0 mr-2">
                                <input placeholder="Buscar..." id="search" name="search" type="number" class="validate" value="{{ $search !== null ? $search : ''}}">
                            </div>
                    ...
                    <tbody>
                        @foreach ($items as $key => $item)
                            ...
                            <tr>
                                ...
                                <td>{{ $budgets[$dataIndex]['etapaorcamento_descricao'] }}</td>
                                <td>
                                    <a href="{{ route('edition.show', $budgets[$dataIndex]['iddocumento']) }}"><i class="material-icons">remove_red_eye</i></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
